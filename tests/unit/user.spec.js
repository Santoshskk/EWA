...
describe('User Model', () => {
  ...
  it('ensures the model has all the important variables', () => {
    const userCopy = User.copyConstructor(user1)
    expect(userCopy.user_id, 'userId is undefined').toBeDefined()
    expect(userCopy.sector_id, 'sectorId is undefined').toBeDefined()
    expect(userCopy.first_name, 'first name is undefined').toBeDefined()
    expect(userCopy.last_name, 'last name is undefined').toBeDefined()
